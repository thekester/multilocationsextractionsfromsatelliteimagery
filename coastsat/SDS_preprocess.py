...
def save_jpg(metadata, settings, use_matplotlib=False):
    ...
    try:
        ...
        print(f"Error: Missing key in settings - {e}")
        return
    ...
    for satname in metadata.keys():
        try:
            ...
            print(f'{satname}: {len(filenames)} images')
        except KeyError as e:
            print(f"Error: Missing key in metadata for satellite {satname} - {e}")
            continue

        # Loop through images
        for i in range(len(filenames)):
            print(f'\r{i+1}/{len(filenames)}', end='')
            ...
            try:
                ...
                im_ms, georef, cloud_mask, im_extra, im_QA, im_nodata = preprocess_single(
                    fn, satname, settings['cloud_mask_issue'], settings['pan_off'], collection, s2cloudless_prob
                )

                # Compute cloud_cover percentage (with no data pixels)
                cloud_cover_combined = np.divide(np.sum(cloud_mask.astype(int)), (cloud_mask.shape[0] * cloud_mask.shape[1]))
                ...
                cloud_cover = np.divide(np.sum(cloud_mask_adv.astype(int)), (np.sum((~im_nodata).astype(int))))
                ...
                print(f"Error processing image {filenames[i]} for satellite {satname} - {e}")
        print('')
    
    # Print the location where the images have been saved
    print(f'Satellite images saved as .jpg in {os.path.join(filepath_data, sitename, "jpg_files", "preprocessed")}')
...
def get_reference_sl(metadata, settings):
    ...
    # if it exists, load it and load the geojson file
    ...
    for i in range(len(filenames)):
        if i >= len(metadata[satname].get('epsg', [])):
            print(f'Metadata lists for satellite {satname} do not have enough entries for index {i}')
            continue
        ...
        try:
            im_ms, georef, cloud_mask, im_extra, im_QA, im_nodata = SDS_preprocess.preprocess_single(
                fn, satname, settings['cloud_mask_issue'], settings['pan_off'], collection, settings['s2cloudless_prob'])
        except IndexError as e:
            print(f'Error in preprocessing image {fn}: {e}')
            print(f'Error details: {e.args[0]}')
            print(f'im_ms shape: {im_ms.shape if "im_ms" in locals() else "N/A"}')
            print(f'georef: {georef if "georef" in locals() else "N/A"}')
            print(f'cloud_mask shape: {cloud_mask.shape if "cloud_mask" in locals() else "N/A"}')
            print(f'im_extra: {im_extra if "im_extra" in locals() else "N/A"}')
            print(f'im_QA: {im_QA if "im_QA" in locals() else "N/A"}')
            print(f'im_nodata shape: {im_nodata.shape if "im_nodata" in locals() else "N/A"}')
            continue
        ...
        # decide if the image is good enough for digitizing the shoreline
        ...
        if skip_image:
            ...
            while 1:
                ...
                pts = plt.ginput(n=50000, timeout=-1, show_clicks=True)
                if len(pts) == 0:
                    print('No points were clicked, skipping this image.')
                    continue
                ...
                pt_input = plt.ginput(n=1, timeout=-1, show_clicks=False)
                if len(pt_input) == 0:
                    print('No points were clicked, skipping this action.')
                    continue
                ...
                try:
                    if pt_input[0][0] > im_ms.shape[1] / 2:
                        ...
                        plt.draw()
                        plt.ginput(n=1, timeout=3, show_clicks=False)
                        plt.close()
                        break
                except IndexError as e:
                    print(f'Error while processing pt_input: {e}')
                    print(f'Error details: {e.args[0]}')
                    print(f'pt_input: {pt_input}')
                    print(f'im_ms shape: {im_ms.shape if "im_ms" in locals() else "N/A"}')
                    continue
    ...
    if len(pts_coords) == 0:
        raise Exception('No cloud free images are available to digitise the reference shoreline, download more images and try again')
