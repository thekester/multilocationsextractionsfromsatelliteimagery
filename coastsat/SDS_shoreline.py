...
# CoastSat modules
from coastsat import SDS_tools, SDS_preprocess, SDS_shoreline
...
def extract_shorelines(metadata, settings):
    ...
    for satname in metadata.keys():
        if not metadata[satname]:
            print(f'No metadata for satellite {satname}')
            continue

        # get images
        filepath = SDS_tools.get_filepath(settings['inputs'], satname)
        filenames = metadata[satname].get('filenames', [])

        if not filenames:
            print(f'No filenames found for satellite {satname}')
            continue
        ...
        for i in range(len(filenames)):
            if i >= len(metadata[satname].get('dates', [])) or i >= len(metadata[satname].get('epsg', [])) or i >= len(metadata[satname].get('acc_georef', [])):
            ...
            try:
                ...
                if settings['adjust_detection']:
                    date = filenames[i][:19]
                    skip_image, shoreline, t_mndwi = SDS_shoreline.adjust_detection(im_ms, cloud_mask, im_nodata, im_labels,
                ...
                # otherwise map the contours automatically with one of the two following functions:
                    ...
                    try:
                        ...
                        print(f'Could not map shoreline for this image: {filenames[i]}. Error: {e}')
                        continue
                    ...
                    # visualise the mapped shorelines
                ...
                print(f'Could not process image {filenames[i]}: {e}')
...
###################################################################################################
# Global variable to control automatic mode
automatic_mode = False

def show_detection(im_ms, cloud_mask, im_labels, shoreline, image_epsg, georef,
                   settings, date, satname):
    global automatic_mode  # Use the global variable to control automatic mode
    """
    Shows the detected shoreline to the user for visual quality control. 
    The user can accept/reject the detected shorelines by using keep/skip buttons or enable automatic mode.
    ...
    # Check cloud cover to skip automatically if in automatic mode
    if automatic_mode:
        cloud_cover = np.sum(cloud_mask) / cloud_mask.size
        if cloud_cover > settings['cloud_thresh']:
            skip_image = True
            print(f"Skipped image '{date}' due to high cloud cover: {cloud_cover:.2f}")
        else:
            skip_image = False
        ...
        while True:
            ...
            btn_skip = plt.text(-0.1, 0.9, '⇦ skip', size=12, ha="left", va="top",
                                transform=ax1.transAxes,
                                bbox=dict(boxstyle="square", ec='k',fc='w'))
            btn_auto = plt.text(0.5, 0.9, 'Auto ⇩', size=12, ha="center", va="top",
                                transform=ax1.transAxes,
                                bbox=dict(boxstyle="square", ec='k',fc='w'))
            ...
            if key_event.get('pressed') == 'right':
                ...
                automatic_mode = True  # Enable automatic mode
